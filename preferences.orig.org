...
Execute this function to disable any history and backups.
...
Browse the blocks at https://www.compart.com/en/unicode/block.
...
(use-package expand-region
  ...
  (add-to-list 'er/try-expand-list #'tree-sitter-mark-bigger-node 'append)

  ;; run mark-next-accessor only if the region is a full symbol.
  ;; this is to prevent the cases of "very_long_[symbol].method(xx)"
  ;; expand into "very_long_[symbol.method](xx)" instead of
  ;; "[very_long_symbol].method(xx)".
  (defun shou/er/mark-next-accessor-advice (orig-fun &rest args)
    (when (< (point) (mark))
      (exchange-point-and-mark))
    (when (looking-at "\\_<.+\\_>")
      (apply orig-fun args)))
  (advice-add 'er/mark-next-accessor :around #'shou/er/mark-next-accessor-advice)

  ;; mark outside double quotation mark.
  ;;
  ;; advantage of this function over er/mark-outside-pairs is that it
  ;; works not based on syntax but based on the actual text. Thus it
  ;; works for texts that are not syntax highlighted (e.g. inside
  ;; comment).
  (defun shou/er/mark-outside-quotes ()
    "Mark the region outside the quotation marks."
    (interactive)
    (let ((region-active (region-active-p))
          ...
          close-quote-pos)
      (if region-active
          (unless (string-match-p "^\".+\"$" (buffer-substring-no-properties region-start region-end))
            (setq open-quote-pos (save-excursion (goto-char region-start) (search-backward "\"") (point)))
            (setq close-quote-pos (save-excursion (goto-char region-end) (search-forward "\"") (point))))
        (setq open-quote-pos (if (looking-at "\"") (point) (save-excursion (search-backward "\"") (point))))
        (setq close-quote-pos (save-excursion (search-forward "\"") (point))))
      (when (and open-quote-pos close-quote-pos)
        (set-mark close-quote-pos)
        (goto-char open-quote-pos))))
  ;; \" ; trick smartparen that there is no mismatch quotation mark
  (add-to-list 'er/try-expand-list #'shou/er/mark-outside-quotes))
#+END_SRC
...
Run =M-x sudo-edit= to edit current file as root.
...
(use-package kubernetes-tramp
  :when (and (f-exists-p "~/.kube/config")
             ;; emacs 29 natively supports kubernetes
...
I'm interested in modal editing but I don't want to learn a new set of keybindings. God mode is a good compromise.
...
(use-package god-mode
  ...
  :config
  (defun shou/enter-insert-mode-for-god-mode (&rest args)
    "advice this after commands you want to switch in insert mode"
    (when (bound-and-true-p god-local-mode)
      (god-local-mode -1)))
  (advice-add 'open-line :after #'shou/enter-insert-mode-for-god-mode)

  (defun shou/god-mode-update-cursor ()
    (setq cursor-type (if (or god-local-mode buffer-read-only)
                          'hollow
                        'box)))
  (add-hook 'god-mode-enabled-hook 'shou/god-mode-update-cursor)
  (add-hook 'god-mode-disabled-hook 'shou/god-mode-update-cursor)
  )
#+end_src
...
(use-package eglot
  ...
  :custom
  ;; auto shutdown a language server when the last buffer it manages
  ;; is closed.
  (eglot-autoshutdown t)
  ;; allow eglot to enable on definition found by xref outside project
  (eglot-extend-to-xref t)

  :bind
  (:map eglot-mode-map
        ("M-l a a" . eglot-code-actions)
        ("M-," . xref-find-definitions)
        ("M-l l" . eglot-inlay-hints-mode)
        ("M-l r" . eglot-rename)
        )

  :config
  (defun shou/disable-eglot-inlay-mode ()
    (eglot-inlay-hints-mode -1))
  ;; disable inlay hint by default, toggle with "M-l l"
  (add-hook 'eglot-managed-mode-hook #'shou/disable-eglot-inlay-mode)
  )
#+end_src

***** eglot-x

various rust-analyzer extensions
...
By default, eglot uses the built-in flymake for displaying errors. However, I found flycheck more customization in various ways. The author of eglot has refused the idea of adding flycheck support, which is understandable. Fortunately some people have written a package to add flycheck support to eglot.
...
(use-package sideline-flycheck
  ...
  :autoload (sideline-flycheck-setup)
...
(use-package sideline-flymake
  ...
  :config
  ;; inhibit eldoc for flymake when sideline-flymake is enabled
  (defun shou/advice-flymake-eldoc-function (orig-fun &rest args)
    (if (memq 'sideline-flymake sideline-backends-right)
        nil
      (apply orig-fun args)))
  (advice-add #'flymake-eldoc-function :around 'shou/advice-flymake-eldoc-function))
#+end_src

*** Debug Adapter Protocol (DAP)

See https://github.com/svaante/dape#c-c-and-rust---codelldb for more setup instructions.
...
(use-package dape
  ...
  :custom
  ;; GUD gdb like
  (dape-buffer-window-arrangment 'gud) ; default: 'left
  :config
  ;; Do not show REPL by default
...
This package was disabled because it's overly strict. I mostly switch over to smartparens.
...
(use-package rust-ts-mode
  ...
  :config
  ;; rust-related compile mode regexp alist, etc.
...
(use-package python
  ...
  :hook (python-mode . shou/python-mode-hook)
  :custom
  (python-shell-completion-native-enable nil)
  :config
  (defun shou/python-mode-hook ()
    ;; step 1: start pye3nv if pyvenv-virtual-env is unbound or empty, and "venv" dir exists
    (when (and (or (not (boundp 'pyvenv-virtual-env))
                   (null pyvenv-virtual-env)
                   (string-empty-p pyvenv-virtual-env))
               (locate-dominating-file "venv" "."))
      (pyvenv-activate (locate-dominating-file "venv" ".")))
...
It's a little difficult to set up. Here's the instructions:

1. launch jupyter server (=jupyter lab=)
2. =M-x jupyter-connect-server-repl=
3. =M-x jupyter-server-launch-kernel=
4. =M-x jupyter-connect-server-repl= (again, select kernel this time)
5. close the repl but do not shut down kernel
6. =C-c C-c= on a =BEGIN_SRC jupyter= block
...
(use-package jupyter
  ...
  (with-eval-after-load 'ob-jupyter
    (setq org-babel-default-header-args:jupyter
          '((:async . "yes") ;; do not block emacs while executing
            (:session . "py") ;; give a common name to the shared session
            (:kernel . "python") ;; use python kernel
            (:display . "image plain html") ;; fallback in this order
            (:pandoc . t) ;; convert html back to org mode
            ))
...
(use-package lean4-mode
  :straight (:host github :repo "leanprover/lean4-mode" :files ("*.el" "data"))
  :config
  (with-eval-after-load 'lsp-mode
...
(use-package wgsl-mode
  :straight (:host github :repo "KeenS/wgsl-mode.el")
  :mode ("\\.wgsl\\'"))
#+end_src
...
(use-package copilot
  ...
  :straight (:host github :repo "raymond-w-ko/copilot.el" :files ("dist" "*.el"))
  ;; :straight (:local-repo "~/tmp/copilot.el" :files ("dist" "*.el"))
  :commands (copilot-mode copilot-clear-overlay copilot-complete)
  :custom
  ;; enable logging (max size: 10000)
  (copilot-log-max 10000)
  (copilot-idle-delay 0.3)
  ;; (copilot-network-proxy '(:host "127.0.0.1" :port 8888))
  ;; no warning if mode-specific offset cannot be found
  ...
  (defun shou/copilot-disable-predicate ()
    (or
     ...
     (string-match-p "\\(?:secret\\|cred\\|hidden\\|private\\)"
  ...
  ;; cached copilot slur regexp, computed on first invocation
  (defvar shou/copilot-slur-regexp nil)
  (defun shou/copilot-slur-regexp nil
    (or (bound-and-true-p shou/copilot-slur-regexp)
        (setq shou/copilot-slur-regexp
              ;; the list comes from https://moyix.net/~moyix/copilot_slurs_rot13.txt
              (let* ((words (->> (f-read "~/.emacs.d/assets/copilot_slurs_rot13.txt")
                                 (rot13)
                                 (s-split "\n")
                                 (-remove (lambda (s)
                                            (or (s-starts-with-p "#" s)
                                                (s-starts-with-p "<" s)
                                                (s-blank-p s)))))))
                (eval `(rx word-start (or ,@words) word-end))))))

  (defun shou/toggle-highlight-copilot-slurs nil
    "Toggle highlight of copilot slurs."
    (interactive)
    (if (bound-and-true-p shou/copilot-slur-on)
        (progn
          (setq-local shou/copilot-slur-on nil)
          (unhighlight-regexp (shou/copilot-slur-regexp)))
      (setq-local shou/copilot-slur-on t)
      (highlight-regexp (shou/copilot-slur-regexp))))

  (defun shou/neuter-copilot-slur-words (text)
    (let ((slur-regexp (shou/copilot-slur-regexp))
          ;; replace the last char with a placeholder, e.g. "hello" => "hell@"
          (censor (lambda (s) (concat (substring s 0 -1) "@"))))
      (s-replace-regexp slur-regexp censor text)))
...
(use-package chatgpt-shell
  ...
  (chatgpt-shell-model-versions '("gpt-4-0314" "gpt-3.5-turbo-0613"))
  (chatgpt-shell-system-prompts
   '(("General" . "Do not give answer promptly unless it's straightforward. Analyze carefully and try to be as accurate as you can. If you don't know the answer or need more information, you should ask for more details. This is very important to my career. You'd better be sure.")
     ("Programming" . "I am an expert in programming. I need you help to answer programming related questions. Do not give answer promptly unless it's straightforward. Analyze the problem carefully and be as accurate as you can. If you don't know the answer or need more information, you should ask for more details. This is very important to my career. You'd better be sure.")
     ("Book" . "I am reading a book. I will send you the portions I'm reading. You help the me 1. summarize and 2. explain what the author is talking. Keep your response short (2-3 sentences). You may use markdown to structure your response. This is very important to my career. You'd better be sure.")
     ))
  (chatgpt-shell-welcome-function #'shou/chatgpt-shell-welcome-function)

  :config
  (setq chatgpt-shell-openai-key (shou/get-pass-entry "openai/api-key"))
  (defun shou/chatgpt-shell-welcome-function (config)
    "Return a string to be displayed when the chatgpt-shell starts."
    (format "Commands: %s, %s, S-<return> (newline)\n\nKeys:\n- %s\t\t%s\n- %s\t\t%s\n\n"
            (propertize "clear" 'font-lock-face 'italic)
            (propertize "help" 'font-lock-face 'italic)
            (propertize "C-c C-v" 'font-lock-face 'font-lock-string-face)
            (propertize "chatgpt-shell-swap-model-version" 'font-lock-face 'font-lock-doc-face)
            (propertize "C-c C-s" 'font-lock-face 'font-lock-string-face)
            (propertize "chatgpt-shell-swap-system-prompt" 'font-lock-face 'font-lock-doc-face)))
  )
...
(defun shou/anki-front-from-calibre-clipboard ()
  "Copy a sentence from Calibre, call this function to convert it to Anki front in org syntax."
  (interactive)
  (let* ((shell-file-name "/bin/sh")
         (html (shell-command-to-string
                "xclip -t text/html -o -sel clipboard 2>/dev/null || true"))
         (no-tags (replace-regexp-in-string "<span\\b[^>]*text-decoration-line:\\s-*underline[^>]*>\\([^<]*\\)</span>" "*/\\1/*" html)))
    (xml-substitute-special (replace-regexp-in-string "<[^>]+?>" "" no-tags))))

(defun shou/anki-extract-focus (text)
  "Get the focus part of the text (delimited by */ and /*)."
  (when (and text (stringp text))
    (let ((first-split (split-string text "\\*/")))
      (when (> (length first-split) 1)
        (let ((second-split (split-string (cadr first-split) "/\\*")))
          (when (car second-split)
            (car second-split)))))))

;; (defun shou/insert-note ()
;;   (interactive)
;;   (tempel-insert 'anki/note))
;; (bind-key "<f7>" 'shou/insert-note)
...
(use-package anki-editor
  ...
  :config
  (defun shou/anki-editor--export-filter-output (contents backend info)
    (string-trim contents))

  (org-export-define-derived-backend 'shou-anki 'ascii
    :filters-alist
    '((:filter-final-output . shou/anki-editor--export-filter-output))
    :translate-alist
    '((bold . (lambda (bold contents _info)
                (format "<b>%s</b>" contents)))
      (italic . (lambda (italic contents _info)
                  (format "<i>%s</i>" contents)))
      (paragraph . (lambda (paragraph contents _info)
                     (replace-regexp-in-string "\n\n" "<br>" contents)))))


  (setq anki-editor--ox-anki-html-backend 'shou-anki))
#+END_SRC
...
#+begin_src emacs-lisp
(setf (alist-get "\\.pdf\\'" org-file-apps) "atril %s")
#+end_src
...
(use-package beancount
  ...
  ;; enable outline folding
  :hook (beancount-mode . outline-minor-mode)
  :hook (beancount-mode . shou/beancount-customization)
  :config
  (defun shou/beancount-customization ()
