...
(use-package jinx
  ...
  (defun shou/jinx-suggestions-from-session-words (orig-f word)
    "Return suggestions for WORD from session words."
    (nconc
     (cl-loop
      for w in jinx--session-words
      if (<= (string-distance word w) 4)
      collect (propertize w 'jinx--group "Suggestions from session words"))
     (funcall orig-f word)))

  (advice-add 'jinx--correct-suggestions
              :around 'shou/jinx-suggestions-from-session-words)
