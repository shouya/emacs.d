...
GUI Emacs on macOS does not inherit the shell environment. =shell.env= captures
it as =KEY=VALUE= lines; load it here so =PATH= and friends are available before
the rest of the config runs.

#+begin_src emacs-lisp
(let ((env-file (expand-file-name "shell.env" user-emacs-directory)))
...
;; elpaca does not automatically update built-in packages, declare manually
...
** =C-c E= to edit current computer's config

#+begin_src emacs-lisp
(defun shou/find-nixos-config ()
  "Find nixos config"
  (interactive)
  (let* ((host (system-name))
         (root "~/projects/user-config")
         (primary (format "hosts/%s/default.nix" host))
...
(bind-key "C-c E" #'shou/find-nixos-config)
#+end_src
...
(defun shou/other-window-or-ace ()
  "Switch to other window. If C-u is pressed, call ace-window."
  (interactive)
...
;; clone the buffer and show it aside, default C-x 4 c
(bind-key "C-x c" #'clone-indirect-buffer-other-window)

(defun shou/other-window-backwards nil (interactive) (other-window -1))
(when (display-graphic-p)
  ;; this binding breaks arrow keys on emacs tui
...
(use-package expand-region
  ...
  :disabled t
  :init
  (setq expand-region-reset-fast-key (kbd "C-0"))
...
I found expreg.el faster and typically more predictable than expand-region.
...
(use-package expreg
  ...
  :bind (("C-=" . expreg-expand)
         ("C--" . expreg-contract))
  :config
  (setq-default expreg--verbose t)

  ;; add list inset
  (defun shou/expreg--trim-whitespaces (regions)
    (mapcan
     (lambda (region)
       (pcase-let ((`(,producer . (,beg . ,end)) region))
         (save-excursion
           ...
           (list ;; (cons (cons 'ws-trimed producer) (cons beg end))
            region))))
     regions))

  (advice-add #'expreg--inside-list :filter-return #'shou/expreg--trim-whitespaces)
  (advice-add #'expreg--tree-sit :filter-return #'shou/expreg--trim-whitespaces)

  ;; inhibit paragraph in org-src
  (defun shou/expreg--trim-inhibit-in-org-src ()
    (not (and (featurep 'org) (eq major-mode 'org-mode) (org-in-src-block-p))))
  (advice-add #'expreg--paragraph-defun :before-while #'shou/expreg--trim-inhibit-in-org-src)

  ;; blank-line delimited block
(defun shou/expreg--balanced-paragraph ()
  "Return the largest line-wise paren-balanced region within the
current paragraph that still contains point's line.
Unmatched parens above point's line advance BEG; unmatched parens
below it retreat END.  Returns nil if no such region exists (e.g.
the imbalance is on point's own line)."
  (save-excursion
    (let ((cur-beg (line-beginning-position))
      ...
      (forward-paragraph)
      (setq end (point))
      (while (and (not done) (< beg end))
        (setq state (parse-partial-sexp beg end))
        (cond
         ;; Minimum depth < 0: an unmatched CLOSER exists.
         ((< (nth 6 state) 0)
          ;; Re-parse with target depth -1: stops right after the
          ;; first unmatched closer.
          (parse-partial-sexp beg end -1)
          (let ((pos (1- (point))))          ; the closer itself
            (cond
             ((< pos cur-beg)                ; above cursor -> BEG down
              (goto-char pos)
              (forward-line 1)
              (setq beg (point)))
             ((> pos cur-end)                ; below cursor -> END up
              (goto-char pos)
              (setq end (line-beginning-position)))
             (t (setq beg end)))))           ; on cursor's line -> fail
         ;; Final depth > 0: unmatched OPENER(s), positions in (nth 9),
         ;; outermost first.
         ((> (car state) 0)
          (let* ((opens (nth 9 state))
                 ;; innermost unmatched opener above the cursor line
                 (above (car (last (seq-filter
                                    (lambda (p) (< p cur-beg)) opens))))
                 ;; outermost unmatched opener below the cursor line
                 (below (seq-find (lambda (p) (> p cur-end)) opens)))
            (cond
             (above (goto-char above)        ; above cursor -> BEG down
                    (forward-line 1)
                    (setq beg (point)))
             (below (goto-char below)        ; below cursor -> END up
                    (setq end (line-beginning-position)))
             (t (setq beg end)))))           ; on cursor's line -> fail
         ;; Region ends inside a string: trim END to the string's line.
         ((nth 3 state)
          (goto-char (nth 8 state))
          (setq end (line-beginning-position)))
         (t (setq done t))))
      (when (and done (< beg end))
        (list (cons 'paragraph (cons beg end)))))))
  (add-to-list 'expreg-functions #'shou/expreg--balanced-paragraph)

  (setq-default expreg-functions expreg-functions) ;; override default value
  )
#+end_src
...
#+begin_src emacs-lisp
;; move point to new copy after duplicating
(setq duplicate-region-final-position -1
      duplicate-line-final-position -1)
(bind-key* "M-j" #'duplicate-dwim)
#+end_src
...
(use-package recentf
  ...
  (recentf-exclude
   (list (rx "bookmarks" eol) ;; or it always shows up first
         ...
         (rx ".cargo/registry") ;; rust library source files
         (rx "/nix/store") ;; nix source files
         (rx "/tmp/fish.") ;; fish commandline editor
         ))
  ;; auto clean up every day after 11:00pm
  (recentf-auto-cleanup "11:00pm")

  :init
  ;; save 2000 files instead of default 20 files in history
  (setq recentf-max-saved-items 2000)

  ;; save every 1 minutes (default: save on emacs exit)
  (defun shou/recentf-save-if-changed ()
    "Save `recentf-list` only if it has changed since last save."
    (let ((last-recentf-list nil))
      (lambda ()
        (unless (equal last-recentf-list recentf-list)
          (let ((inhibit-message t)) (recentf-save-list))
          (setq last-recentf-list recentf-list)))))
  (run-at-time 60 60 #'shou/recentf-save-if-changed)
...
(use-package org
  ...
  ;; treat as symbols than pairs (fix compairson signs & pair matching)
...
** Jump to emacs function definition

#+begin_src emacs-lisp
(bind-key "M-f" #'find-function help-map)
(bind-key "M-k" #'find-function-on-key help-map)
#+end_src
...
(defun shou/enable-sandbox ()
  ...
  (with-eval-after-load 'project
    (advice-add 'project--remember-dir :filter-args
                (lambda (args) (cons (car args) '(t))))) ;; set nowrite
...
** Automatic revert on file change
...
(use-package popwin
  ...
  (push '(crit-list-mode :position bottom :stick t) popwin:special-display-config)
  (push '("*Flycheck error messages*" :position bottom :height 10 :noselect t)
...
#+begin_src emacs-lisp
(bind-key "C-M-p" '(lambda() (interactive) (scroll-other-window -1)))
(bind-key "C-M-n" '(lambda() (interactive) (scroll-other-window 1)))
...
#+begin_src emacs-lisp
;; Preserve external clipboard content before killing in emacs to
;; allow yanking from the kill ring.
(setq save-interprogram-paste-before-kill t)

;; No duplicate entries that waste space
...
(use-package consult
  :ensure t
  :after recentf compat
  :bind
  (("M-g g" . consult-goto-line)
   ;; consult single line
   ("M-g M-g" . consult-line)
   ("M-g f" . shou/consult-diagnostics)
   ;; any org heading
  ...
  :custom
  ;; only show preview when pressing C-.
  ...
  ;; adapted from https://github.com/minad/consult/issues/6#issuecomment-848077449
  ;; trigger via C-x b p <SPC>
  ...
  (add-to-list 'consult-buffer-sources 'consult--source-project-file)

  (defun shou/consult-diagnostics (arg)
    "Consult diagnostics produced by lsp or flycheck"
    (interactive "P")
    (cond
     ((and lsp-mode
           (not (hash-table-empty-p (lsp-diagnostics))))
      (consult-lsp-diagnostics arg))
     (flycheck-mode (consult-flycheck))))
  )
  ...
  (defun shou/jinx-valid-english-possessive-p (start)
    (let ((word (buffer-substring-no-properties start (point))))
      (and (string-match-p "['’]s\\'" word)
           (jinx--word-valid-p (substring word 0 -2)))))
  (defun shou/jinx-valid-math-expression-p (start)
    (let ((word (buffer-substring-no-properties start (point))))
      ;; matches: π, a², ℕ, Δt, xⱼ, Ā, etc
      (string-match-p "\\`[Α-Ωα-ω℀-⅏Ā-ſ][²-ₜⱼ]*\\|[A-Za-z][²-ₜⱼ]+\\|[Α-Ωα-ω][A-Za-z]\\'" word)))

  (add-to-list 'jinx--predicates #'shou/jinx-valid-english-possessive-p)
  (add-to-list 'jinx--predicates #'shou/jinx-valid-math-expression-p)
  )
#+end_src
...
(use-package tempel
  ...
  (defun shou/tempel-snippet (file &optional replacements)
    "Read snippet FILE from `tempel-snippet-path` and return its contents as a string.

If REPLACEMENTS is non-nil, it should be an alist of (FROM . TO) strings.
Each FROM string is replaced with the corresponding TO string throughout
the snippet text. Replacements are applied sequentially in the order they
appear in the alist."
    (let ((text (f-read-text (f-join tempel-snippet-path file))))
      (cl-loop
       for (from . to) in replacements
       do (setq text (string-replace from to text)))
      text)
  )
)
#+end_src
...
(use-package tramp
  :custom
  ;; cache file names for 3 minutes
  (remote-file-name-inhibit-cache 180)
  ;; avoid locking for remote files
  (remote-file-name-inhibit-locks t)
  ;; do not store remote command history over tramp
  (tramp-histfile-override t)

  ;; increase inline transfer threshold, use out of band (rsync) for larger files
  (tramp-copy-size-limit (* 1024 1024)) ;; 1MiB
  ;; use rsync for larger files
  (tramp-use-scp-direct-remote-copying t)
  (tramp-default-method "rsync")

  ;; treat as timeout after 1 sec
  (remote-file-name-access-timeout 1)
  ...
  ;; use direct async for remote processes (used by e.g. magit)
  ...
  ;; do not disable ssh control master for compile commands
...
*** Highlight nested pairs with colors
...
Typing on opening delimiter inserts closing delimiter. Wrap pair around active region.

#+begin_src emacs-lisp
(electric-pair-mode 1)

;; inhibit electric pair when:
(setq electric-pair-inhibit-predicate
      (lambda (c)
        (or
         (minibufferp)                  ; don't insert pair in minibuffer
         (not (eolp))                   ; don't insert pair unless point is at eol
         (electric-pair-default-inhibit c))))
#+end_src

*** Deleting pair

Put cursor on (or before) the opening pair and hit =C-M-z= to delete the pair.

#+begin_src emacs-lisp
(setopt delete-pair-blink-delay 0.1)    ; delete pair immediately
(when (version<= "31.1" emacs-version)
  (setopt delete-pair-push-mark t)      ; use C-u C-SPC to jump to recently deleted closing pair
  )
(bind-key* "C-M-z" #'delete-pair)
#+end_src
...
#+begin_src emacs-lisp
(bind-key* "C-x ;" #'comment-line)      ; comment code
(bind-key* "M-;" #'comment-dwim)        ; add comment
#+end_src
...
(use-package keyfreq
  :ensure t
  ;; avoid tty emacs session locking on the same file
  :when (display-graphic-p)
...
#+BEGIN_SRC emacs-lisp
(defun scratch (buffer-name)
  "open a scratch buffer"
  (interactive
   (list (completing-read "Buffer name: "
                          (list (concat (make-temp-name "scratch-") ".org")
                                (concat (make-temp-name "scratch-private-") ".org")
                                (concat (make-temp-name "scratch-") ".txt")
                                )
                          nil nil "scratch-")))
  (let* ((buffer (generate-new-buffer buffer-name))
         (scratch-dir (f-expand "~/tmp/scratch"))
         (target-file (format "%s/%s" scratch-dir buffer-name)))
...
(use-package xclip
  ...
  :custom
  (xclip-method (or
                 (and (eq system-type 'darwin) 'pbpaste)
                 (and (getenv "DISPLAY") 'xclip)
                 (and (getenv "WAYLAND_DISPLAY") 'wl-copy)))
  )
#+end_src
...
The ~select-active-regions~ will attempt to save everything selected to the system clipboard. After manually copying a region of text, the region will dissolve back to a cursor. Although there is no active region, a empty string is save into the system clipboard anyway, overriding the previous copied content. Disabling the copy-active selection should fix the issue.
...
#+begin_src emacs-lisp
(defun shou/modify-number-at-point (fn)
  "Apply FN to the number at point and replace it with the result.

FN takes one integer argument (the number found at point) and
returns a new integer.  The replacement preserves the original
number’s leading zeros when the new value fits in the same number
of digits.  If the new value requires more digits, the width is
expanded naturally.

Signals an error if no number is found at point."
  (save-excursion
    (let* ((bounds (bounds-of-thing-at-point 'number))
      ...
      (let* ((orig-num (buffer-substring-no-properties start end))
             (orig-len (- end start))
             (num (string-to-number orig-num))
             (new-text (number-to-string (funcall fn num))))
        (when (and (< (length new-text) orig-len)
                   (string-prefix-p "0" orig-num))
          (setq new-text (concat (make-string (- orig-len (length new-text)) ?0)
                                 new-text)))
        (delete-region start end)
        (insert new-text)))))

(defun shou/increment-at-point ()
  "Increment the number at point by 1.

Preserves leading zeros when the incremented number still fits
the original width.  For example: 01 → 02, 09 → 10."
  (interactive)
  (shou/modify-number-at-point (lambda (x) (1+ x))))

(defun shou/decrement-at-point ()
  "Decrement the number at point by 1.

Preserves leading zeros when the decremented number still fits
the original width.  For example: 10 → 09."
  (interactive)
  (shou/modify-number-at-point (lambda (x) (1- x))))

(defun shou/modify-number-at-point-expr (expr)
  "Modify the number at point by evaluating EXPR with `x' bound to that number.

EXPR is read as Elisp and evaluated.  The result must be an
integer.  Leading zeros are preserved when the resulting value
fits in the original number's width."
  (interactive "sExpression (use x): ")
  (shou/modify-number-at-point
   (lambda (x) (eval (read expr)))))

(bind-key "<f2>" nil) ;; free up the <f2> key space (previously used by 2C-mode commands)
(bind-key "<f2> a" #'shou/increment-at-point)
(bind-key "<f2> x" #'shou/decrement-at-point)
(bind-key "<f2> e" #'shou/modify-number-at-point-expr)
#+end_src
...
(use-package org
  ...
  :custom
  ;; partial dates after today resolves to the current year
...
(use-package ox-pandoc
  ...
  (defun shou/ox-mediawiki-transform ()
    ...
    ;; 3. convert [BROKEN LINK: White Nights] into [[White Nights]]
    (goto-char (point-min))
    (while (re-search-forward "\\[BROKEN LINK: \\(.*?\\)\\]" nil t)
      (let ((link-text (match-string 1)))
        (replace-match (format "[[%s]]" link-text))))
...
(use-package magit
  ...
  ;; Hit enter while visiting a blob file (from history) goes to the
  ;; file in worktree
  (bind-key "RET" #'magit-blob-visit-file 'magit-blob-mode-map)

  ;; rebind M-w for copying things (section revision, selection, etc)
  (bind-key "M-w" #'magit-copy-section-value 'magit-mode-map)
  )
#+end_src
...
#+begin_src emacs-lisp
(use-package crit
  :ensure (:host nil :repo "https://git.lain.li/shouya/crit.el.git")
  ;; :ensure (:repo "~/projects/crit" :files (:defaults))
  :bind-keymap ("C-x C-r" . crit-prefix-map)
  :config
  (crit-persistence-mode))
#+end_src
...
#+begin_src emacs-lisp
(use-package majutsu
  :ensure (:host github :repo "0WD0/majutsu" :files (:defaults))
  :bind
  (("C-x G" . majutsu)
   :map majutsu-blob-mode-map
   ("RET" . majutsu-blob-visit-file))

  :config
  ;; promote status display before log
  (setq majutsu-log-sections-hook
        (->> majutsu-log-sections-hook
            (remove #'majutsu-log-insert-status)
            (cons #'majutsu-log-insert-status)
            (remove #'majutsu-log-insert-error-header)
            (cons #'majutsu-log-insert-error-header)))
  )
#+end_src
...
(use-package project
  :ensure nil
  :custom
  (project-vc-merge-submodules nil) ;; ignore files from submodules
  (project-vc-include-untracked t)  ;; include new, untracked files
  :bind-keymap
  ("M-`" . project-prefix-map)
  :bind
  (:map project-prefix-map
        ("g" . shou/project-switch-to-magit))
  :config
  (require 'magit-extras) ;; inserts (m) magit to project-switch-commands
  (setq project-switch-commands
        '((project-find-file "Find file")
          (project-dired "Dired")
          (magit-project-status "Magit")))

  (defun shou/project-switch-to-magit nil
    "Switch to a project and call magit, regardless if in a project or not"
    (interactive)
...
(use-package rg
  ...
  (defun shou/rg-rerun-toggle-word ()
    "Rerun last search with toggled word-regexp."
    (interactive)
    (rg-rerun-toggle-flag "--word-regexp"))

  ;; do not search lines with over 240 columns
  (setq rg-initial-toggle-flags '("--max-columns=240" "--word-regexp")))
#+end_src
...
(use-package lsp-mode
  ...
  :init
  ;; lsp-enable-snippet detects (fbound 'yas-minor-mode) before
  ;; enabling snippet. make it believe we have yasnippet installed.
  (fset 'yas-minor-mode #'ignore)
  ...
  (defun shou/lsp-mode-setup-completion ()
    ...
    ;; remove dabbrev completion
    (remove-hook 'completion-at-point-functions #'cape-dabbrev t)

    ;; ensure tempel matches before lsp-completion
    (remove-hook 'completion-at-point-functions #'lsp-completion-at-point t)
    (add-hook 'completion-at-point-functions #'lsp-completion-at-point 90 t)
    )
...
*** Stub yasnippet with tempel

#+begin_src emacs-lisp
(use-package lsp-snippet-tempel
  :ensure (:host github :repo "svaante/lsp-snippet")
  :commands lsp-snippet-tempel-lsp-mode-init
...
(use-package dumb-jump
  ...
  ;; unmark as obsolete, i need this function from time to time in lsp-mode.
...
(use-package sideline-lsp
  ...
  (setq sideline-lsp-code-actions-prefix
        (concat (all-the-icons-octicon "light-bulb" :height 0.7 :v-adjust -0.0575)
                " ")))
#+end_src

** Highlight/edit all occurrences
...
| =C-'=         | toggle hide unrelated lines |
| =C-;=         | toggle single occurence     |
| =M-M=         | convert into mc             |
| =C-h C-h=     | help                        |
...
#+begin_src emacs-lisp
;; On nix-darwin, /etc/zshenv rebuilds PATH in every zsh unless
;; =__NIX_DARWIN_SET_ENVIRONMENT_DONE= is set, which would overwrite the
;; direnv-injected PATH in shell-mediated subprocesses (compile,
;; shell-command, cargo-process, ...).
...
(use-package direnv
  ...
  :init
  ;; ensure env updated before lsp command spawn
  (defun shou/direnv-update-env (&rest args)
    (direnv-update-environment))
  (with-eval-after-load 'lsp-mode
    (advice-add 'lsp-mode :before #'shou/direnv-update-env))
...
#+begin_src emacs-lisp
(use-package track-changes :ensure (:source "MELPA")) ;; newer version required by copilot
(use-package copilot
  ;; faster emacs -nw load time (for quick edits)
  :when (display-graphic-p)
  ...
  (copilot-max-char 1000000) ;; increase to 1M
  ...
  :config
  ;; (add-hook 'copilot-mode-hook #'copilot-nes-mode)

  (defun shou/buffer-file-name ()
    "Return the file name of the current buffer (or the base buffer if in narrowed buffer)."
    (or (buffer-file-name)
  ...
  (cl-defun shou/copilot-disable-predicate ()
    (let ((buffer-file (shou/buffer-file-name)))
      ...
      (or
       ...
       (string-match-p "\\(?:secret\\|cred\\|hidden\\|private\\|auth\\|app-2024\\)" buffer-file)
       ;; do not enable copilot on gpg/age files
       (string-match-p "\\.\\(gpg\\|asc\\|age\\)\\'" buffer-file)
       (string-match-p "\\.bean\\'" buffer-file)
       (string-match-p "org/\\(plan\\|projects\\).org" buffer-file)
       ;; exclude large buffer of pure texts,
       ;; copilot is very inaccurate.
       (and (derived-mode-p 'text-mode) (> (buffer-size) 60000))
       ;; do not enable copilot on files in tmp/cu
       (string-match-p "tmp/cu" buffer-file)
       ;; do not enable for inline command line editing
       (string-match-p "/tmp/fish." buffer-file)
       )))
...
#+begin_src emacs-lisp
(use-package wingman
  :ensure (wingman :host github :repo "mjrusso/wingman")
  :when (display-graphic-p)

  ;; Disabled because the local fill-in-middle models are yet to reach
  ;; a satisfactory level of usability.
  :disabled
  :hook
  (prog-mode . wingman-mode)
  (text-mode . wingman-mode)
  :bind
  (:map wingman-mode-prefix-map
        ...
        ("M-f" . wingman-accept-word))

  :init
  (setq wingman-prefix-key (kbd "C-c w"))

  :config
  (setq wingman-log-level 2)
  (setq wingman-ring-n-chunks 16)

  ;; default llama.cpp server port is 8012
  (setq wingman-llama-endpoint "http://127.0.0.1:11434/infill")
  (setq wingman-model-name "fim")
)
#+end_src
...
(use-package mediawiki-mode
  ...
  (defun shou/default-date-from-firefox ()
    (let* ((command
            (cond
             ((equal (getenv "XDG_CURRENT_DESKTOP") "niri")
              "niri msg windows | grep -i -e firefox -e wiki")
             ((not (null (getenv "DISPLAY")))
              "wmctrl -l -x | grep -i firefox | grep -i wiki")))
           (line (shell-command-to-string command))
...
(use-package rust-mode
  ...
  ;; set default indent size to 4. Actual indent size will be auto-detected
...
(use-package rust-mode
  ...
  (:map rust-mode-map
        ...
        ("C-h d" . lsp-rust-analyzer-open-external-docs)
        )
...
(use-package agenix
  :ensure t
  :custom
  (agenix-key-files '("~/.ssh/age_id_ed25519" "~/.ssh/id_ed25519"))
  )
#+end_src
...
(use-package python
  :custom
  ;; the default pep-257 style is rather ugly.
...
(use-package kdl-mode
  :ensure t
  :config
  (add-hook 'kdl-mode-hook #'shou/set-tab-width-to-4)
  (defun shou/set-tab-width-to-4 nil (setq-local tab-width 4))
  )
#+end_src
