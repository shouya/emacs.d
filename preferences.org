...
I now use vertico for minibuffer completion. It's considerable faster than ivy.
...
;;   :config
;;   (add-hook 'minibuffer-setup-hook #'vertico-repeat-save 99)
;;   )
#+end_src
...
(use-package proof-general
  :mode ("\\.v\\'" . coq-mode)
  :hook (coq-mode . shou/enable-company-coq)
  ...
  ;; Coq mode completion:
  ;; putting it under proof-general because it loads company-mode and yasnippet which I don't currently use. I will only load it when necessary.
  (use-package company-coq
    :commands shou/enable-company-coq
    :after proof-general
    :config
...
#+begin_src emacs-lisp
(use-package mu4e
  ;; I don't use emacs as email client anywhere else
  :if (and (executable-find "mu")
           (executable-find "offlineimap"))
  :commands mu4e
  :ensure nil
  :custom
  (mu4e-sent-folder "/tubi/[Gmail].Sent Mail")
  (mu4e-drafts-folder "/tubi/[Gmail].Drafts")
  (mu4e-trash-folder "/tubi/[Gmail].Trash")
  (mu4e-refile-folder "/tubi/[Gmail].All Mail")
  ;; update email with U
  ...
  :config
  (setq user-mail-address (string-trim-right (shell-command-to-string "pass show tubi/email")))
  (setq user-full-name (string-trim-right
                        (shell-command-to-string "pass show tubi/gmail/given-name")))
  )
#+end_src
...
(use-package mu4e-alert
  :after mu4e
  :hook
  (emacs-startup . mu4e-alert-enable-notifications)
  (emacs-startup . mu4e-alert-enable-mode-line-display)
  :custom
  (mu4e-alert-interesting-mail-query
   "flag:unread maidir:/tubi/INBOX")
