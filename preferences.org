...
(use-package expand-region
  ...
  ;; mark outside double quotation mark. region may or may not be active.
  ;;
  ;; case 1: region not active
  ;; if current char is quote, find the matchig one.
  ;; if current char is not a quote, look outwards (both backwards and forwards)
  ;; to find the enclosing quotation mark.
  ;;
  ;; case 2: region active
  ;; if the region is already quoted, return nil
  ;; if the region is not quoted, search backward (from the start) to find open quote and forward (from the end) to find close quote
  ;;
  ;; mark the region from the open quote to close quote (inclusive)
  ;;
    ...
    (let ((region-active (region-active-p))
      ...
      (if region-active
          (if (string-match-p
              ...
              nil
            (let ((open-quote-pos (save-excursion
                                    (goto-char region-start)
                                    (search-backward "\"")
                                    (point)))
                  (close-quote-pos (save-excursion
