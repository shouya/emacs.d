...
(use-package apheleia
  ...
  :config
  (add-to-list 'apheleia-formatters '(mix . ("mix" "format" file)))
...
(use-package org
  :custom
  (org-refile-targets '(("projects.org" . (:tag . "PROJECT"))
                        ("mencti.org" . (:level . 1))
                        ))
...
#+BEGIN_SRC emacs-lisp
(setq shou/previously-clocking nil)
...
(defun shou/resume-previous-clock ()
  (interactive)
  (if (null shou/previously-clocking)
      (message "No previous clock is available.")
    (let ((id (cdr shou/previously-clocking)))
      (save-window-excursion
        (org-id-goto id)
        (org-clock-in)
        (setq shou/previously-clocking nil)))))
...
text and copying to the killring."
    ...
    (when (eq major-mode 'org-mode) ; do this only in org-mode buffers
      ...
      (setq mytmphead (replace-regexp-in-string org-link-bracket-re "\\2" mytemphead))
