...
(use-package aider
  ...
  :custom
  (aider-args '("--model" "gpt-4o" "--edit-format" "diff"))
  )
...
(use-package aidermacs
  ...
  ;; --watch-files doesn't work otherwise
...
(use-package nix-mode
  ...
  (when (executable-find "home-manager")
    (setq lsp-nix-nixd-home-manager-options-expr
          (format "(builtins.getFlake \"%s\").homeConfigurations.\"shou@mrnix\".options"
                  (f-canonical "~/.config/home-manager"))))
  (when (executable-find "nixos-version")
    (setq lsp-nix-nixd-nixos-options-expr
          (format "(builtins.getFlake \"%s\").nixosConfigurations.mrnix.options"
                  (f-canonical "/etc/nixos")))))
#+end_src
