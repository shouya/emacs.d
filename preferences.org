...
(defun shou/get-pass-entry (entry)
  ;; because auth-source-pass-get depends on the auto-mode-alist for
  ;; gpg files, which is purposely unset on boot to speed up the load
  ;; time.
...
(use-package idris2-mode
  ...
  (advice-add #'idris2-complete-at-point :around #'cape-wrap-silent)
  (advice-add #'idris2-complete-at-point :around #'cape-wrap-purify)
  )
#+END_SRC
...
I now prefer GitHub Copilot over TabNine.
...
Instruct the AI to modify codes.

#+begin_src emacs-lisp
(use-package codex-completion
  :straight (codex-completion :type git :host github :repo "debanjum/codex-completion")
  :bind ("C-x C-r" . codex-completion-instruct)
  :custom
  (codex-completion-openai-model "gpt-3.5-turbo")
  :config
  ;; this step is a little slow (because of decryption of password
  ;; store). so we put in in config block to make it load on-demand.
  (setq codex-completion-openai-api-token (shou/get-pass-entry "openai/api-key")))
#+end_src
