...
(use-package company
  ...
  :custom
  (company-idle-delay 0.3 "Show completion almost immediately")
  (company-show-numbers t "Number the candidates (use M-1, M-2 etc to select completions)")
  (company-tooltip-align-annotations t)
  (company-dabbrev-code-everywhere t)

  ;; only enable these options
  (company-backends '(company-dabbrev-code company-capf company-files))
...
(use-package eglot
  ...
  :custom
  ;; do not confirm when applying code actions
  (eglot-confirm-server-initiated-edits nil)

  ;; do not tamper company-backends
  (eglot-stay-out-of '(company))

  ;; use async connection only
...
(use-package elixir-mode
  :straight (:host github :repo "elixir-editors/emacs-elixir" :flavor nil)
...
(use-package rust-mode
  ...
  (use-package cargo
    ...
    :config
    (add-hook 'rust-mode-hook 'cargo-minor-mode))r
...
(use-package org-mru-clock
  :bind (("C-c C-x C-j" . org-clock-jump-to-current-task)
