...
;;   ;; use circled number for eyebrowse workspace name
;;   (spaceline-workspace-numbers-unicode t)
;;
;;   ;; change color based on evil state
;;   (spaceline-highlight-face-func 'spaceline-highlight-face-evil-state)
;;
;;   :config
;;
;;   (setq modeline-left-side
;;         '(
;;           (workspace-number
;;            :fallback evil-state
;;            :face highlight-face
...
;;            :priority 9)
;;           ((point-position line-column buffer-position)
;;            :priority 7)
;;           (major-mode :priority 9)
;;           (process :when active)
;;           ((flycheck-error flycheck-warning flycheck-info) :when active :priority 8)
;;           (version-control :when active :priority 5)
;;           ))
;;
;;   (setq modeline-right-side
;;         '((org-clock :priority 9)
;;           (which-function
;;            (purpose :priority 7)
;;            (selection-info :priority 7)
;;            (global :when active)
;;            )))
;;
;;   (apply 'spaceline-compile 'shou
;;          modeline-left-side
;;          modeline-right-side)
;;   (setq mode-line-format '("%e" (:eval (spaceline-ml-shou)))))
#+END_SRC
...
** Window config switching

#+BEGIN_SRC emacs-lisp
; (use-package eyebrowse
;   :hook (emacs-startup . eyebrowse-mode)
;   :config
;   ;; use M-0..M-9 to switch config
...
(use-package persp-mode
  ...
  (setq persp-keymap-prefix (kbd "M-n")))
#+END_SRC
...
(use-package popwin
  ...
  :config
  (push '("*elixir-format-errors*" :noselect t)
        popwin:special-display-config)
  (push '("*idris-repl*")
        popwin:special-display-config)
  (push '("*idris-holes*" :noselect t)
        popwin:special-display-config))
#+END_SRC
...
(use-package evil-surround
  ...
  (dolist (x '((?\( . '("(" . ")"))
               ...
               (?\] . '("[ " . " ]"))
               (?\} . '("{ " . " }"))))
    (add-to-list 'evil-surround-pairs-alist x)))
#+END_SRC
...
(use-package org
  ...
  (org-agenda-sorting-strategy
   '((agenda category-keep habit-up todo-state-up priority-down time-up)
...
(use-package org
  ...
  :custom
  (org-capture-templates `(;; Work task
                           ("w" "Work task" entry (file+headline "todo.org" "Work")
                            "* TODO %^{Task (Work)}\nSCHEDULED: %t\n%?\nAdded: %U\n")

                           ;; Personal task
                           ("p" "Personal task" entry (file+headline "todo.org" "Personal")
                            "* TODO %^{Task (Personal)}\nSCHEDULED: %t\n%?\nAdded: %U\n")
                           ...
                           ("S" "Clocking task/finished Step" checkitem (clock)
                            "- [X] %^{Finished step}\n"
                            :immediate-finish t :empty-lines-before t)
                           ...
                           ;; Date plan entry
                           ("d" "Date plan for today" checkitem (file+olp+datetree "plan.org")
                            "- [ ] %a%?\n"
                            :jump-to-captured t)
                           )))
#+END_SRC
...
(use-package org-mru-clock
  :bind (("C-c C-x C-j" . org-color-jump-to-current-task)
